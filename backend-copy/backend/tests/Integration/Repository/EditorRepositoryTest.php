...
class EditorRepositoryTest extends TestCase
{
    ...
    public function testFindByNameLogic(): void
    {
        ...
        $editor1->setHeadOffice('Paris, France');

        $editor2 = new Editor();
        $editor2->setName('Flammarion');
        $editor2->setCreationDate(new \DateTime('1876-01-01'));
        $editor2->setHeadOffice('Paris, France');
    ...
    public function testFindByHeadOfficeLogic(): void
    {
        ...
        $editors = [
            (new Editor())->setName('Gallimard')->setCreationDate(new \DateTime('1911-05-31'))->setHeadOffice('Paris, France'),
            (new Editor())->setName('Flammarion')->setCreationDate(new \DateTime('1876-01-01'))->setHeadOffice('Paris, France'),
            (new Editor())->setName('Penguin Books')->setCreationDate(new \DateTime('1935-01-01'))->setHeadOffice('Londres, Royaume-Uni'),
            (new Editor())->setName('Random House')->setCreationDate(new \DateTime('1927-01-01'))->setHeadOffice('New York, USA'),
        ];
        ...
        $filteredEditors = array_filter($editors, function($editor) use ($searchHeadOffice) {
            return $editor->getHeadOffice() === $searchHeadOffice;
        });

        // Vérifications
        $this->assertCount(2, $filteredEditors);
        foreach ($filteredEditors as $editor) {
            $this->assertEquals('Paris, France', $editor->getHeadOffice());
        }
    }
    ...
    public function testFindByCreationDateLogic(): void
    {
        ...
        $editors = [
            (new Editor())->setName('Ancien Éditeur')->setCreationDate(new \DateTime('1800-01-01'))->setHeadOffice('Ville Ancienne'),
            (new Editor())->setName('Moderne Éditeur')->setCreationDate(new \DateTime('2000-01-01'))->setHeadOffice('Ville Moderne'),
            (new Editor())->setName('Éditeur Classique')->setCreationDate(new \DateTime('1900-01-01'))->setHeadOffice('Ville Classique'),
        ];
        ...
        $filteredEditors = array_filter($editors, function($editor) use ($searchDate) {
            return $editor->getCreationDate() >= $searchDate;
        });

        // Vérifications
        $this->assertCount(2, $filteredEditors);
        foreach ($filteredEditors as $editor) {
            $this->assertGreaterThanOrEqual($searchDate, $editor->getCreationDate());
        }
    }
    ...
    public function testSortByNameLogic(): void
    {
        ...
        $editors = [
            (new Editor())->setName('Z Publishing')->setCreationDate(new \DateTime('2000-01-01'))->setHeadOffice('Ville Z'),
            (new Editor())->setName('Alpha Books')->setCreationDate(new \DateTime('1900-01-01'))->setHeadOffice('Ville A'),
            (new Editor())->setName('Beta Press')->setCreationDate(new \DateTime('1950-01-01'))->setHeadOffice('Ville B'),
        ];
    ...
    public function testFindWithBooksLogic(): void
    {
        ...
        $editor->setCreationDate(new \DateTime('1911-05-31'));
        $editor->setHeadOffice('Paris, France');
    ...
    public function testFindByCreationPeriodLogic(): void
    {
        ...
        $editors = [
            (new Editor())->setName('Éditeur XIXe')->setCreationDate(new \DateTime('1850-01-01'))->setHeadOffice('Ville XIXe'),
            (new Editor())->setName('Éditeur XXe Début')->setCreationDate(new \DateTime('1920-01-01'))->setHeadOffice('Ville XXe Début'),
            (new Editor())->setName('Éditeur XXe Fin')->setCreationDate(new \DateTime('1980-01-01'))->setHeadOffice('Ville XXe Fin'),
            (new Editor())->setName('Éditeur XXIe')->setCreationDate(new \DateTime('2010-01-01'))->setHeadOffice('Ville XXIe'),
        ];
        ...
        $filteredEditors = array_filter($editors, function($editor) use ($startPeriod, $endPeriod) {
            $creationDate = $editor->getCreationDate();
        ...
        $this->assertCount(2, $filteredEditors);
        foreach ($filteredEditors as $editor) {
            $this->assertGreaterThanOrEqual($startPeriod, $editor->getCreationDate());
            $this->assertLessThanOrEqual($endPeriod, $editor->getCreationDate());
        }
    }
    ...
    public function testAdvancedSearchByCountryAndBookCountLogic(): void
    {
        ...
        $editor2->setHeadOffice('Londres, Royaume-Uni');

        $editor3 = new Editor();
        $editor3->setName('Flammarion');
        $editor3->setCreationDate(new \DateTime('1876-01-01'));
        $editor3->setHeadOffice('Paris, France');
        ...
        $filteredEditors = array_filter($editors, function($editor) {
            return strpos($editor->getHeadOffice(), 'Paris') !== false && $editor->getBooks()->count() >= 1;
        });

        // Vérifications
        $this->assertCount(2, $filteredEditors);
        foreach ($filteredEditors as $editor) {
            $this->assertStringContainsString('Paris', $editor->getHeadOffice());
    ...
    public function testPaginationLogic(): void
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $editor->setCreationDate(new \DateTime("1900-01-01"));
            $editor->setHeadOffice("Ville $i");
            $editors[] = $editor;
        }
    ...
    public function testFindByAgeLogic(): void
    {
        ...
        $editors = [
            (new Editor())->setName('Très Ancien')->setCreationDate(new \DateTime('1800-01-01'))->setHeadOffice('Ville Ancienne'),
            (new Editor())->setName('Ancien')->setCreationDate(new \DateTime('1900-01-01'))->setHeadOffice('Ville Ancienne'),
            (new Editor())->setName('Moderne')->setCreationDate(new \DateTime('2000-01-01'))->setHeadOffice('Ville Moderne'),
        ];
        ...
        $filteredEditors = array_filter($editors, function($editor) use ($hundredYearsAgo) {
            return $editor->getCreationDate() <= $hundredYearsAgo;
        });

        // Vérifications
        $this->assertCount(2, $filteredEditors);
        foreach ($filteredEditors as $editor) {
            $this->assertLessThanOrEqual($hundredYearsAgo, $editor->getCreationDate());
        }
    }
}
