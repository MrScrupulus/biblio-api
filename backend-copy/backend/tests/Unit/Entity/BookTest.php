...
class BookTest extends TestCase
{
    ...
    /**
     * Test de la relation avec Editor
     */
    ...
    /**
     * Test qu'on peut mettre l'éditeur à null
     */
    public function testSetEditorToNull(): void
    {
        // D'abord on met un éditeur
        $editor = new Editor();
        $this->book->setEditor($editor);
        
        // Puis on le remet à null
        $this->book->setEditor(null);
        
        $this->assertNull($this->book->getEditor());
    }
    ...
    public function testCompleteBookSetup(): void
    {
        ...
        $editor->setCreationDate(new \DateTime('1945-01-01'));
        $editor->setHeadOffice('New York, USA');
    ...
    /**
     * Test avec des caractères spéciaux dans l'image
     */
    public function testImageWithSpecialCharacters(): void
    {
        $imagePath = 'images/livres/l\'été-2024/book-cover.jpg';
        $this->book->setImage($imagePath);
        
        $this->assertEquals($imagePath, $this->book->getImage());
    }

    /**
     * Test de la relation bidirectionnelle avec Author
     */
    public function testBidirectionalAuthorRelation(): void
    {
        ...
        // Utiliser la méthode addBook pour établir la relation bidirectionnelle
        $author->addBook($this->book);

        // Vérifier que l'auteur connaît ce livre
        $this->assertTrue($author->getBook()->contains($this->book));
        $this->assertEquals($author, $this->book->getAuthor());
    }

    /**
     * Test de la relation bidirectionnelle avec Editor
     */
    public function testBidirectionalEditorRelation(): void
    {
        $editor = new Editor();
        $editor->setName('Flammarion');
        $editor->setCreationDate(new \DateTime('1876-01-01'));
        $editor->setHeadOffice('Paris, France');

        // Utiliser la méthode addBook pour établir la relation bidirectionnelle
        $editor->addBook($this->book);

        // Vérifier que l'éditeur connaît ce livre
        $this->assertTrue($editor->getBooks()->contains($this->book));
        $this->assertEquals($editor, $this->book->getEditor());
    }
}
